...
int _heightOfSubTree(Node* root){
	...
	if(root->left==NULL && root->right==NULL)
		...
		int l=0, r=0;
		if(root->left!=NULL)
			l = _heightOfSubTree(root->left) + 1;
		if(root->right!=NULL) 
			r = _heightOfSubTree(root->right) + 1;
...
TYPE _removeLeftMost(Node* curr, Node* parent, Node* origAncestor){
	...
	TYPE removeThisVar;
	if(curr==NULL){
		removeThisVar = parent->val; 
		origAncestor->right = parent->right;
	}
	else{
...
Node* _removeNodeFromSubtree(Node* curr, TYPE val){
	...
	if(_compare(val,curr->val)==0){
		if(curr->right==NULL && curr->left==NULL)
			...
			curr->val = _removeLeftMost(leftmost, parent, curr);	
		}
	}  
    else if(_compare(val,curr->val)==-1)
