...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	for(i = 0; i < source->numNeighbors; ++i)
		if(!source->neighbors[i]->isVisited)
			if(DFSRecursiveHelper(g, source->neighbors[i], destination))
				return 1;
	return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while(!isEmptyListDeque(&queue))
	{
		for ( i = 0; i < frontListDeque(&queue)->numNeighbors; ++i)
		{
			if ( frontListDeque(&queue)->neighbors[i]->isVisited == 0)
			{
				if (frontListDeque(&queue)->neighbors[i] == destination
					|| frontListDeque(&queue) == destination)
				{
					isFound = 1;
					break;
				}
				addBackListDeque(&queue,  frontListDeque(&queue)->neighbors[i]);
				frontListDeque(&queue)->neighbors[i]->isVisited = 1;
			}
		}
		removeFrontListDeque(&queue);
		if (isFound)
			break;
	}
	freeListDeque(&queue);
...
void computeAdjacencyMatrix(Graph* g)
{
	...
	int vertIdx;
	/* allocate mem*/
	matrix = (int **)malloc(g->numVertices * sizeof(int *));
	for (i = 0; i < g->numVertices; ++i)
	{
		matrix[i] = (int *) malloc(g->numVertices * sizeof(int));
		memset(matrix[i], 0, g->numVertices * sizeof(int));
	}
	/* compute*/
	for(i = 0; i < g->numVertices; ++i)
	{
		vert = &g->vertexSet[i];
		for(j = 0; j < vert->numNeighbors - 1; ++j)
		{
			vertIdx = vert->neighbors[j]->label - 'A';
			matrix[i][vertIdx] = 1;
		}
